...
import { CreateClassDto, RegisterClassDto } from '@dto'
...
export class ClassesService {
  ...
  async registerStudent(classId: number, registerDto: RegisterClassDto): Promise<ClassRegistration> {
    ...
    if (classEntity.registrations.length >= classEntity.max_students) throw new BadRequestException('Class is full')

    const existingRegistration = classEntity.registrations.find((reg) => reg.student_id === registerDto.student_id)

    if (existingRegistration) throw new BadRequestException('Student is already registered for this class')
    ...
    const conflictingClass = studentRegistrations.find(
      (reg) => reg.class.day_of_week === classEntity.day_of_week && reg.class.time_slot === classEntity.time_slot,
    )
